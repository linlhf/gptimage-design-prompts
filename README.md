...
你可以给它文字需求，也可以配合草图、平面图、模型截图、现场照片、材质参考、情绪板或多张参考图。它会输出适合复制到 ChatGPT Images / GPT Image 2 的英文主 prompt、中文意图说明、使用步骤和推荐参数。

## 怎么提问才能让 skill 听懂

最少要说清楚两件事：**输入是什么** + **想要什么**。其他能补就补，不补也行。

### 最简版（够用）

「这是一张 [输入类型]，帮我转成 [目标] 的 prompt」

例子：
- 这是一张手绘景观平面图，帮我转成彩色总平面的 prompt
- 这是 SU 模型截图，帮我转成 MIR 风格建筑渲染的 prompt
- 这是一张街角老建筑照片，帮我转成现代咖啡馆改造的 prompt
- 这是情绪板 + 后院照片，帮我把情绪板里的植物和材质用到后院上

### 加细节（更稳）

如果有以下信息，加进去会让出图更可控：

- **必须保留**：树木位置、相机角度、建筑体量、路径关系等
- **可以改**：材质、植物、家具、灯光、氛围
- **风格倾向**：MIR 渲染 / 竞赛图板 / CAD 线稿 / 极简室内
- **最终用途**：作品集 / 竞赛 / 客户沟通

例子：

> 这是一张 SU 模型截图，帮我转成建筑渲染 prompt。保留相机角度和体量关系；材质是玻璃幕墙 + 浅色混凝土 + 木质入口；要 MIR 风格，柔和日光，作品集用。

### 多图任务有个额外要求

如果你上传 2 张及以上参考图，**告诉 skill 每张图各自的角色**，否则它会猜错。比如：

> 第一张是植物情绪板（只用来选植物种类和材质），第二张是后院现状照片（保留布局和相机角度）。

这是 skill 里 Multi-Image Role Declaration 规则的输入对应物，跳过这一步是多图任务最大的翻车原因。
